...
def speech_recognizer(source, language = 'en-US'):
    text = ""
    logger.info('end of speech, translating >>>>>')
    try:
        text = r.recognize_google(source, language = language)
...
# Load your audio.
audio_file = AudioSegment.from_wav("mine.wav")
print(f'The number of channels is: {audio_file.channels}')
print(f'The length of the audio file is {len(audio_file)}')
print(f'Duration in seconds is: {audio_file.duration_seconds}')

logger.info('Calculating total silence time >>>>>>')
print(f'Total silence time intervals are: {silence.detect_silence(audio_file, min_silence_len=1000, silence_thresh=-40)}')
...
#Calculating extended silence
for start, end in (silence):
    difference = end - start
    if difference > 5:
        print(f'There\'s an extended silence between the second {start} and {end}')
...
for i, chunk in enumerate(chunks):
    ...
    transcribed_text = speech_recognizer(audio, language)
